...
export default function passwordChangeScreenStd() {
    ...
    const onBtnPress = async () => {
        ...
        const loginCheck = await db.select().from(admin).where(and(eq(admin.username, user.username), eq(admin.password, currentPassword)));
        ...
        const res = await db.update(admin).set({ password: newPassword }).where(eq(admin.username, user.username));
        ...
        router.replace('/PlatformRoute/(tabs)/home');
        return;
